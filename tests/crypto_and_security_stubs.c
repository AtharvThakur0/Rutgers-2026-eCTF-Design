/**
 * @file crypto_and_security_stubs.c
 * @brief Host-side no-op stubs for crypto and security functions referenced by
 *        secure_design.c.  Used only in the secure_blob_store unit-test build,
 *        which never calls the stub implementations.
 */
...
bool security_secure_uart_check_and_update_replay(
    uint32_t Nr, uint32_t Nt, uint16_t group_id, uint8_t op
) {
    (void)Nr; (void)Nt; (void)group_id; (void)op;
    return false;
}
