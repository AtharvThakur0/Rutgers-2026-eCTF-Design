 ...
 *   - "HSM locked\n"  when pin_is_locked() is true before OR after check_pin()
 *   - "Invalid pin\n" when the PIN is simply wrong (< 5 failures total)
 *
 * Sending distinct error messages lets the host tool (and test scripts)
 * distinguish a permanent lockout from a transient wrong-PIN rejection.
 */
static int pin_gate(unsigned char *pin) {
  ...
  if (!check_pin(pin)) {
    /* check_pin() may have incremented fail_count to PIN_MAX_FAIL */
    if (pin_is_locked()) {
      print_error("HSM locked\n");
    } else {
 ...
 *
 *  Reads whatever payload arrived with the ECHO command and writes it
 *  straight back in an ECHO response.  No authentication, no crypto.
